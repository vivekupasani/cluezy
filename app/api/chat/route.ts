  ...
  try {
    ...
    if (userId === 'anonymous') {
      ...
      const { success, limit, remaining } =
        await unauthenticatedRateLimit.limit(identifier)
    ...
    if (isSharePage) {
      return new Response(
        'Chatting is disabled on shared links. Start a new conversation to continue.',
        {
          status: 403,
    ...
    const transformedApps = selectedApps.map((app: string) =>
      app.replace(/-/g, '')
    )
