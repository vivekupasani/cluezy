import { getUserIdByCustomer, getUserPlan, revokeUserPlan, saveUserPlanDetails } from '@/lib/actions/user-premium';
    ...
    try {
        switch (event.type) {
            case 'checkout.session.completed': {
                ...
                if (!userId) {
                    console.error("No client_reference_id (userId) found in session. User may not have been logged in during checkout.");
            ...
            case 'customer.updated': {
                ...
                if (userId) {
                    ...
                    if (existingData) {
                        ...
                        console.log(`Updated customer details for user ${existingData.email}`);
                    } else {
                        console.error(`Could not find existing premium data for userId: ${userId} during customer.updated`);
                    }
                } else {
                    console.error(`Could not find userId for customer: ${customerId} during customer.updated`);
                }
                break;
            }
            ...
            case 'customer.subscription.updated': {
                ...
                if (userId) {
                    ...
                    if (existingData) {
                        ...
                        console.log(`Updated premium plan to ${planName} for user ${existingData.email}`);
                    } else {
                        console.error(`Could not find existing premium data for userId: ${userId}`);
                    }
                } else {
            ...
            case 'customer.subscription.deleted': {
                ...
                if (userEmail) {
                    ...
                    console.log(`Revoked premium access for user ${userEmail} (customer: ${customerId})`);
                } else {
                    console.error(`Could not find email for subscription deletion. Customer: ${customerId}`);
                }
