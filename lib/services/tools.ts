...
export async function getVercelTools(userId: string, toolkits: string[]) {
            ...
            try {
              ...
              if (
                ...
                return {
                  ...
                  suggestion:
                    'Please try again with more specific filters (e.g., date range, keywords) or a smaller limit/max_results parameter (e.g., set it to 5 or less).'
                }
              }
  ...
  console.log(
    '✅ Finished fetching tools. Total unique tools mapped:',
    Object.keys(mappedTools).length,
    'from total fetched:',
    totalToolsFetched
  )
  return mappedTools
...
export async function authorizeToolKit(
  ...
  const connectionRequest = await session.authorize(toolName, {
...
export async function disconnectToolKit(userId: string, toolkitSlug: string) {
  ...
  if (
    ...
    console.log(
      `Toolkit ${toolkitSlug} is not connected or no active account found.`
    )
    return false
  }
}
