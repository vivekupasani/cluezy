...
export const youtubeVideoAnalysisTool = tool({
  description:
    'Get detailed analysis of a specific YouTube video including captions, timestamps, and full metadata. Use when user provides a specific YouTube URL.',
  parameters: z.object({
    ...
    try {
      ...
      try {
        ...
        const [details, subs] = await Promise.all([
          getVideoDetails({ videoID: videoId, lang: 'en' }).catch(
            (e: unknown) => {
        ...
        if (
          ...
          console.log(
            `✅ Got captions from details: ${transcriptText.length} characters`
          )
        } else if (subs && Array.isArray(subs) && subs.length > 0) {
          transcriptText = subs.map((s: any) => s.text).join('\n')
          console.log(
            `✅ Got captions from API: ${transcriptText.length} characters`
          )
        } else {
        ...
        const generateChaptersFromSubtitles = (
          ...
          const parseSeconds = (s: string) =>
            Number.isFinite(Number(s)) ? Number(s) : 0
          const lastSub = subs[subs.length - 1]
          const totalDurationSec = Math.max(
            0,
            parseSeconds(lastSub.start) + parseSeconds(lastSub.dur)
          )
          ...
          for (let t = interval; t < totalDurationSec; t += interval) {
            ...
            chapters.push(
              `${timeString} - ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`
            )
