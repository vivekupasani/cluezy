...
export const weatherTool = tool({
  ...
  parameters: z.object({
    ...
    latitude: z
      ...
      .optional()
      .describe(
        'The latitude coordinate. Required if location is not provided.'
      ),
    longitude: z
      .number()
      .optional()
      .describe(
        'The longitude coordinate. Required if location is not provided.'
      )
  }),
  execute: async ({
    location,
    ...
    try {
      ...
      if (!location && (!latitude || !longitude)) {
        throw new Error(
          'Either location name or both latitude and longitude coordinates must be provided'
        )
      }
      ...
      if (!lat || !lng) {
        if (!location) {
          throw new Error(
            'Location name is required when coordinates are not provided'
          )
        }
