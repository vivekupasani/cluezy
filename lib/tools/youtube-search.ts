...
export const youtubeSearchTool = tool({
  description:
    'Search YouTube videos using Exa AI. Returns basic video information for search results.',
  parameters: z.object({
  ...
  execute: async ({
    ...
    try {
      ...
      console.log(
        '🎥 YouTube Search Results found:',
        searchResult.results.length
      )

      // Process results to extract basic video information
      const processedResults: VideoSearchResult[] = searchResult.results.map(
        (result: any) => {
      ...
      console.log(
        `✅ YouTube Search Complete: ${processedResults.length} videos found`
      )
