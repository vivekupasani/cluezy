...
import { CONNECTOR_CONFIGS, ConnectorProvider, PROVIDER_MAP, TOOLKIT_MAP } from './types';
...
export async function createConnection(provider: ConnectorProvider, userId: string) {
    ...
    const connectionRequest = await authorizeToolKit(userId, toolkitSlug, redirectUrl);

    console.log(`✅ ${CONNECTOR_CONFIGS[provider].name} connection request created`);
...
export async function getConnection(provider: ConnectorProvider, userId: string) {
    ...
    try {
        ...
        console.log(`✅ Found connection for ${provider}:`, {
            id: accountId,
            email: (connection as any)?.connectedAccount?.connectionParams?.email || 'connected',
        ...
        return {
            id: accountId,
            provider: provider,
            email: (connection as any)?.connectedAccount?.connectionParams?.email || 'Connected Account',
            createdAt: (connection as any)?.connectedAccount?.createdAt,
        };
    } catch (error) {
...
export async function listUserConnections(userId: string) {
    try {
        ...
        return connectedToolkits.items.map((toolkit: any) => {
            ...
            const connection = toolkit.connection;
            const accountId = connection?.connectedAccount?.id || connection?.id || toolkit.slug;
...
export async function manualSync(provider: ConnectorProvider, userId: string) {
    console.log(`🔄 Manual sync for Composio is handled automatically during tool calls: ${provider}`);
