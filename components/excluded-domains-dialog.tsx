...
export function ExcludedDomainsDialog({
  ...
  const saveDomains = (domains: string[]) => {
    ...
    window.dispatchEvent(
      new CustomEvent('excluded-domains-updated', { detail: domains })
    )
  }
    ...
    <Dialog open={showDialog} onOpenChange={setShowDialog}>
      ...
      <DialogContent className="w-[95%] md:w-full max-w-2xl h-[50vh] sm:h-[60vh] p-0 bg-popover backdrop-blur-sm text-popover-foreground border border-border rounded-2xl overflow-hidden flex flex-col gap-0 cosmic-glass HiddenScrollbar">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Manage domains to exclude from search results
          </DialogDescription>
        </DialogHeader>
          ...
          {excludedDomains.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center select-none">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1 max-w-[200px]">
                Add domains above to prevent them from appearing in search
                results.
              </p>
            </div>
          ) : (
