...
export function ChatArtifactContainer({
  ...
  return (
    <div className="flex-1 min-h-0 h-svh flex relative">
      <div
        className={`absolute p-2 md:p-2 z-50 bg-background/50 backdrop-blur-lg md:bg-transparent md:backdrop-blur-none w-full flex ${open && !isMobileSidebar ? 'justify-end' : 'justify-between'} items-center pointer-events-none`}
      >
        {(!open || isMobileSidebar) &&
          !pages.includes(pathName) &&
          (!isMobile || !pathName.includes('/connectors')) && (
            <motion.div
                ...
                <TooltipTrigger asChild>
                  <div className="flex items-center justify-center">
                    <Search
                      ...
                      className="cursor-pointer text-foreground font-medium hover:bg-accent hover:text-accent-foreground rounded-full"
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="text-xs ml-2 mt-3">
        ...
        <div
          className={cn(
            'flex items-center gap-2 pointer-events-auto mt-0',
            open && !isMobileSidebar
              ? 'mt-2 transition-all duration-300 ease-in-out'
              : 'mt-0 transition-all duration-300 ease-in-out'
          )}
        >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <Ghost
                    ...
                    className={cn(
                      'text-foreground',
                      state.isIncognito && 'text-accent-foreground'
                    )}
                  />
                </Button>
              </TooltipTrigger>
              ...
              >
                <p className="text-xs w-56">
                  {state.isIncognito ? 'Disable' : 'Enable'} incognito: create
                  anonymous threads that aren't saved to your history
                </p>
              </TooltipContent>
            </Tooltip>
          )}
        ...
        {state.isIncognito && (
          <div className="absolute top-16 md:top-12 left-1/2 md:left-1/2 -translate-x-1/2 md:-translate-x-1/2 z-30 pointer-events-none">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-accent/50 dark:bg-secondary/50 border border-dashed border-primary/40 dark:border-primary/60 backdrop-blur-sm animate-in fade-in slide-in-from-top-4 duration-500 fade-out slide-out-to-top-4">
              <Ghost
                size={14}
                className="text-accent-foreground dark:text-secondary-foreground"
              />
              <span className="text-[10px] md:text-[11px] font-medium text-accent-foreground dark:text-secondary-foreground uppercase tracking-wider">
