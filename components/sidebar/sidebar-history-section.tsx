...
export default function SidebarHistorySection() {
    ...
    return (
        <div className="space-y-1 px-2">
            ...
            {groups.thisWeek.length + groups.lastWeek.length < 5 && groups.thisMonth.length > 0 && (
                <>
                    {groups.thisMonth.slice(0, 5 - groups.thisWeek.length - groups.lastWeek.length).map((chat) => (
                        <ChatMenuItemSidebar key={chat.id} chat={chat} />
                    ))}
                </>
            )}

            {groups.thisWeek.length + groups.lastWeek.length + groups.thisMonth.length < 5 && groups.older.length > 0 && (
                <>
                    {groups.older.slice(0, 5 - groups.thisWeek.length - groups.lastWeek.length - groups.thisMonth.length).map((chat) => (
                        <ChatMenuItemSidebar key={chat.id} chat={chat} />
                    ))}
                </>
            )}
