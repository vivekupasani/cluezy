...
export default function UserMenu({ user, state }: UserMenuProps) {
  ...
  return (
    <DropdownMenu>
            ...
            <Avatar
              className={cn(
                ...
                userPlanDetails?.isActive &&
                  userPlanDetails?.planName === 'Pro' &&
                  'ring-2 ring-primary',
                userPlanDetails?.isActive &&
                  userPlanDetails?.planName === 'Max' &&
                  'ring-2 ring-primary'
            ...
            {userPlanDetails?.isActive && (
              <span
                className={cn(
                  ...
                  userPlanDetails?.planName === 'Max' &&
                    'bg-gradient-to-r from-primary to-primary/90',
                  !['Pro', 'Max'].includes(userPlanDetails?.planName || '') &&
                    'bg-primary'
                )}
              >
      ...
      <DropdownMenuContent
        className="w-56 ml-6 border dark:border-border/50 rounded-lg"
        align="end"
      >
        {/* Profile */}
        ...
        {externalLinks.map((link, idx) => (
          <DropdownMenuItem key={idx} onClick={handleLogout} className="">
            <Link
              href={link.href}
              target="_blank"
