...
export const WeatherSection = ({ tool }: WeatherSectionProps) => {
  const data =
    tool.state === 'result' ? (tool.result as WeatherData) : undefined
  ...
  const chartData = data.list.slice(0, chartDataPoints).map(forecast => ({
    name: new Date(forecast.dt * 1000)
      .toLocaleTimeString('en-US', {
        hour: 'numeric',
        hour12: true
      })
    ...
    <div className="w-[95%] md:w-full mx-auto rounded-2xl bg-background mt-4">
      ...
      <div className="bg-muted dark:bg-card boring-dark:bg-card border border-border rounded-2xl p-4 sm:p-5 md:p-6">
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-xs text-muted-foreground">
          ...
          <span className="whitespace-nowrap">
            H: {formatTempForDisplay(currentWeather.main.temp_max)}
          </span>
          <span className="hidden sm:inline">•</span>
          <span className="whitespace-nowrap">
            L: {formatTempForDisplay(currentWeather.main.temp_min)}
          </span>
        </div>
      </div>
